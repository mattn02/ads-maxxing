...
export function ResearchView({ session, busy, send, create, action }: {
  ...
  return <>
    ...
    {!!research.products?.length && <><h2>Products <span className="muted">{research.products.length} researched</span></h2><div className="source-grid">{research.products.map(product => <div className="card source-card" key={product.id}>
      ...
      <div className="images">{research.assets?.filter(asset => product.assetIds.includes(asset.id)).map(asset => <img key={asset.id} src={asset.originalUrl} alt={`Source photo of ${product.title}`} loading="lazy" />)}</div>
      {!product.assetIds.length && <p className="notice">No verified gallery yet. Assign a matching photo from Unsorted below.</p>}
