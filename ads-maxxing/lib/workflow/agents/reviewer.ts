...
export function codeChecks(variant: Variant, image: Buffer): CodeCheck[] {
  ...
  const modernCommercial = [4, 5, 6, 7].includes(variant.rendererVersion ?? -1);
  ...
  return [
    ...(variant.rendererVersion === undefined ? [] : [
      ...
      { name: "renderer_version", passed: [2, 3, 4, 5, 6, 7].includes(variant.rendererVersion), detail: "Supported deterministic renderer version." },
...
export async function reviewAd(variant: Variant): Promise<Review> {
  ...
  if (variant.rendererVersion !== undefined) {
    let fits = false;
    try {
      if ([4, 5, 6, 7].includes(variant.rendererVersion)) await validateCreative(variant.brief, variant.research);
      else { designSchema.parse(variant.brief.design); brandTokensSchema.parse(variant.brief.tokens); }
      validatePlan(variant.brief, [4, 5, 6, 7].includes(variant.rendererVersion) ? variant.research : undefined);
      fits = true;
  ...
  if ([2, 3, 4, 5, 6, 7].includes(variant.rendererVersion ?? -1) && !original) throw new WorkflowError("Saved original is missing; fidelity review cannot run.");
