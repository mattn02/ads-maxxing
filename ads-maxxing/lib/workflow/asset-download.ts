...
export async function downloadImage(input: string, provider = false): Promise<Buffer> {
  ...
  for (let redirects = 0; redirects < 5; redirects++) {
    ...
    const response = await new Promise<http.IncomingMessage>((resolve, reject) => {
      ...
      const request = transport.get(url, { headers: { Accept: "image/png,image/jpeg,image/webp,image/gif", "Accept-Encoding": "identity" }, lookup: (_host, _options, callback) => callback(null, address.address, address.family), signal: AbortSignal.timeout(remaining) }, resolve);
    ...
    if (response.statusCode !== 200 || Number(response.headers["content-length"] ?? 0) > MAX_BYTES) { response.destroy(); throw new WorkflowError("Image is unavailable or exceeds 20 MB."); }
    const chunks: Buffer[] = []; let size = 0;
    for await (const chunk of response) { const bytes = Buffer.from(chunk); size += bytes.length; if (size > MAX_BYTES) { response.destroy(); throw new WorkflowError("Image exceeds 20 MB."); } chunks.push(bytes); }
