...
async function repairGeneratedCopy<T extends z.infer<typeof generatedDraftSchema>>(draft: T, research: Research, context: Record<string, unknown>): Promise<T> {
  ...
  for (let repair = 0; repair <= 2; repair++) {
    ...
    const repaired = await structuredResult({
      model: workflowModel("concierge"), schema: repairSchema,
...
async function refinementCandidate(parent: Variant, feedback: string) {
  return structuredResult({ model: workflowModel("concierge"), schema: generatedRefinementSchema,
...
export async function draftCampaignBrief(research: Research, preferences: Record<string, string>): Promise<BriefInput> {
  ...
  const generated = await structuredResult({ model: workflowModel("concierge"), schema: generatedDraftSchema,
  ...
  const offer = eligibleOffersForProduct(research, product.id)[0];
