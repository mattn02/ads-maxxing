...
async function main() {
  ...
  // A tall, deliberately uncropped fixture proves containment in the square visual slot.
  const visual = Buffer.from(await new ImageResponse(createElement("div", { style: { width: 240, height: 400, background: "#eab3be", display: "flex", alignItems: "center", justifyContent: "center", border: "16px solid #943749" } }, "PRODUCT"), { width: 240, height: 400 }).arrayBuffer());
  ...
  for (const template of ["copy-top", "photo-top"] as const) {
    for (const dark of [false, true]) {
      ...
      const png = await renderCreative({ brief, research, tokens: brief.tokens!, visualBytes: process.argv[3] ? await readFile(process.argv[3]) : visual, logoBytes: { bytes: logo, width: 120, height: 24 } });
